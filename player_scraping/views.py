...
def home(request):
    ...
    startingRoster = user_team.starting_roster.filter() if user_team else []

    # Render the home template with the players context variable
    return render(
        request, "index.html", {"starting_roster": startingRoster}
...
def my_team(request):
    user_team = Team.objects.filter(owner=request.user).first()
    roster = list(user_team.starting_roster.all() if user_team else [])
    roster.extend(user_team.backup_roster.all() if user_team else [])
    logger.debug(user_team.name)
